...
class UserLoginUtils {
	...
	public static function authen($username, $password) {
		...
		if (isset ( $UsersLogin [0] )) {
			...
			$criDept->condition = "user_id = " . $UsersLogin [0]->id . "";
			$labInfo = LabRegister::model ()->findAll ( $criDept );
			if (isset ( $labInfo [0] )) {
				$_SESSION ['REGISTER_INFO'] = serialize ( $labInfo [0] );
			}
	...
	public static function getRegisterInfo() {
		if (isset ( $_SESSION ['REGISTER_INFO'] )) {
			return unserialize ( $_SESSION ['REGISTER_INFO'] );
		} else {
			return null;
		}
	}
