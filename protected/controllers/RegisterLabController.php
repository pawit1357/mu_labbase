...
class RegisterLabController extends CController {
	public $layout = '_blank';
	private $_model;
	public function actionIndex() {
		
// 		MailUtil::sendMail ( 'pawit1357@hotmail.com', 'test', 'xxxxhiiiiiwwww');
		...
		if (isset ( $_POST ['LabRegister'] )) {
			...
			if ($user->save ()) {
				...
				// $model->update_date = date ( "Y-m-d H:i:s" );
				// $model->lab_login_password = md5 ( $model->lab_login_password );
				
				$fac = MFaculty::model ()->findbyPk ( $model->faculty_id );
				$dep = MDepartment::model ()->findbyPk ( $model->dep_department_id );
				$build = MBuilding::model ()->findbyPk ( $model->building_id );
				$labtype = MLabType::model ()->findbyPk ( $model->lab_type_id );
				
				if (isset ( $fac ) && isset ( $dep ) && isset ( $build ) && isset ( $labtype )) {
					$model->lab_code = $fac->code . '-' . $dep->code . '-' . $build->code . '-' . $model->lab_floor . '-' . $labtype->code;
					$_SESSION ['labCode'] = $model->lab_code;
				}
				
				if ($model->save ()) {
					$strSubject = "=?UTF-8?B?" . base64_encode ( ConfigUtil::getEmailSubject () ) . "?=";
					MailUtil::sendMail ( $user->email, $strSubject, self::GetConfirmEmailTemplate ( $model->lab_code, $user->title->name, $user->first_name, $user->last_name, $user->username, $pwd, $fac->name, $dep->name, $build->name, $model->lab_floor, $labtype->name, $model->user_id ) );
				}
			}
			...
			$transaction->commit ();
			// echo '';
// 			$this->render ( '//registerlab/result_2' );
			$this->redirect ( array (
					'RegisterLab/Result',
					'user_id' => $user->id,
					'pwd' => $pwd 
	...
	public function actionResult($user_id, $pwd) {
		...
		$this->render ( '//registerlab/result', array (
				'data' => $model,
				'user' => $user,
				'pwd' => $pwd 
		) );
	...
	public function actionResult1($user_id, $pwd) {
		...
		$this->render ( '//registerlab/result_1', array (
				'data' => $model,
				'user' => $user,
				'pwd' => $pwd 
		) );
											...
											<tbody>
												<tr>
													<td valign="top">
														...
														style="font-weight: bold">' . $lab_code . '</font>
														<br></br> <font color="green">
														( ' . $fac . ' ' . $dep . ' ' . $build . ' ' . $labtype . ' )
							...
							<tbody>
								<tr>
									<td valign="top" style="font-size: 12px; padding: 20px;">
										...
										<a
										href="http://myapps1357.com/labbase/index.php/RegisterLab/Result1/user_id/' . $user_id . '/pwd/' . $pwd . '">Click</a>
									</td>
								</tr>
							</tbody>
						</table>
